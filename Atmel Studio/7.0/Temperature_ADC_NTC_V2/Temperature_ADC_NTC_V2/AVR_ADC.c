...
void AFF_temperature(float temp, uint16_t adc_value)
{
	uint8_t uniter = 0;
	uint8_t dizaine = 0;
	uint8_t virgule = 0;
	...
	dizaine = temp / 10;
	uniter = temp - dizaine*10;
	virgule = temp*10 - dizaine*100 - uniter*10;
	
	if (adc_value < 235)
	{
		Uart_PutString("T = -");
		
		UART_PutChar(dizaine + 0x30);
		UART_PutChar(uniter + 0x30);
		UART_PutChar(0x2E);
		UART_PutChar(virgule + 0x30);
		Uart_PutString(" C");
		UART_PutChar(0x2A);
	} 
	else
