...
SD_RETURN_CODES sd_init(void){
	...
	while(SD_Response[0] != 0x01){
		...
		sprintf(msgUart02, "%d;", SD_Response[0]);
	...
	_SPCR &= ~(1 << SPR01); // set clock rate fosc/2
	...
	{
		sprintf(msgUart03, "%d;", SD_Response[0]);
...
static void SPI_Init(void){
	...
	_SPCR = (1 << SPE0) | (1 << MSTR0);

	// Start with a lower speed for older cards (100-400kHz)
	_SPSR = (1 << 0);
	_SPCR |= (1 << SPR01); // set clock rate fosc/32
}
