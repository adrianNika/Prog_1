...
**************************************************************/
// Volume
/*use*/MOUNT_RESULT FAT_mountVolume(void);
/*
...
// Other
/*use*/uint8_t FAT_createTimeMilli(void);
/*use*/uint16_t FAT_createTime(void);
/*use*/uint16_t FAT_createDate(void);

// Directory
/*use*/FRESULT FAT_makeDir(const char* path);
/*use*/FRESULT FAT_openDir(DIR* dir_p, const char* path);
/*
FRESULT FAT_openDirByIndex(DIR* dir_p, FILE_SD* finfo_p, uint16_t idx);*/
FRESULT FAT_dirBack(DIR* dir_p);

/*use*/FRESULT FAT_findByIndex(DIR* dir_p, FILE_SD* finfo_p, uint16_t idx);
/*use*/FRESULT FAT_findNext(DIR* dir_p, FILE_SD* finfo_p);
/*uint16_t FAT_dirCountItems(DIR* dir_p);*/
/*use*/static FRESULT _FAT_dirRegister(const char* path, uint8_t task);

// File
/*use*/FRESULT FAT_makeFile(const char* path);
/*use*/FRESULT FAT_fwriteFloat(FILE_SD* fp, float nr, uint8_t nrOfDecimals);
/*use*/FRESULT FAT_fwriteInt(FILE_SD* fp, INT_SIZE nr);
/*use*/FRESULT FAT_fwriteString(FILE_SD* fp, const char* string);
/*use*/FRESULT FAT_fwrite(FILE_SD* fp, const void* buff, uint16_t btw, uint16_t* bw);
/*FRESULT FAT_ftruncate(FILE_SD* fp);*/
/*use*/FRESULT FAT_fdelete(DIR* dir_p, uint16_t idx);
/*use*/FRESULT FAT_fdeleteByName(DIR* dir_p, const char* fname);
/*use*/FRESULT FAT_fsync(FILE_SD* fp);
/*use*/FRESULT FAT_fopen(DIR* dir_p, FILE_SD* file_p, char *file_name);
/*FRESULT FAT_fopenByIndex(DIR* dir_p, FILE_SD* file_p, uint16_t idx);*/
/*use*/static void _FAT_freset(FILE_SD* file_p);
/*
bool FAT_feof(FILE_SD* fp);
uint8_t FAT_ferror(FILE_SD* fp);
void FAT_fclear_error(FILE_SD* fp);
char* FAT_getFilename(void);
uint16_t FAT_getItemIndex(DIR* dir_p);
FSIZE_t FAT_getFileSize(FILE_SD* finfo_p);
uint16_t FAT_getWriteYear(FILE_SD* finfo_p);
uint8_t FAT_getWriteMonth(FILE_SD* finfo_p);
uint8_t FAT_getWriteDay(FILE_SD* finfo_p);
uint8_t FAT_getWriteHour(FILE_SD* finfo_p);
uint8_t FAT_getWriteMinute(FILE_SD* finfo_p);
uint8_t FAT_getWriteSecond(FILE_SD* finfo_p);
bool FAT_attrIsFolder(FILE_SD* finfo_p);
bool FAT_attrIsFile(FILE_SD* finfo_p);
bool FAT_attrIsHidden(FILE_SD* finfo_p);
bool FAT_attrIsSystem(FILE_SD* finfo_p);
bool FAT_attrIsReadOnly(FILE_SD* finfo_p);
bool FAT_attrIsArchive(FILE_SD* finfo_p);*/

/*use*/void FAT_fseekEnd(FILE_SD* fp);
/*use*/void FAT_fseek(FILE_SD* fp, FSIZE_t fptr);
/*
bool FAT_feof(FILE_SD* fp);
uint8_t FAT_ferror(FILE_SD* fp);
void FAT_fclear_error(FILE_SD* fp);
...
/*use*/static uint8_t _FAT_nextFileCluster(FILE_SD* file_p);

// System
/*use*/static void _FAT_removeChain(CLSTSIZE_t cluster);
/*use*/static FRESULT _FAT_updateFileInfo(FILE_SD* fp, uint8_t task);
/*use*/static FRESULT _FAT_allocateCluster(CLSTSIZE_t* new_cluster, CLSTSIZE_t new_cluster_val);
/*use*/static void _FAT_moveWindow(DIR* dir_p, CLSTSIZE_t start_cluster);
/*use*/static FRESULT _FAT_clearCluster(CLSTSIZE_t cluster, bool first_sector);
/*use*/static CLSTSIZE_t FAT_tableFindFree(uint8_t task);
/*use*/static unsigned char ChkSum(unsigned char *pFcbName);
/*use*/static void _FAT_stringLFN(uint16_t start_idx, uint16_t length, const char** data, uint8_t data_size);
/*use*/static void _FAT_fillBufferArray(uint16_t start_idx, uint16_t length, uint8_t value);
/*use*/static void _FAT_getSFN(uint8_t entry_pos, uint8_t task, char buff[]);
/*use*/static FRESULT _FAT_followPath(DIR* dir_p, const char* path, uint8_t task);
/*static FRESULT _FAT_createSFN(DIR* dir_p, const char* source, char dest[], bool* needs_lfn);*/
/*use*/static FRESULT _FAT_getFileNextSector(DIR* dir_p);
/*use*/static uint16_t MATH_remainder(uint16_t divident, uint16_t divisor, uint16_t quotient);
/*use*/static SECTSIZE_t _FAT_clusterToSector(CLSTSIZE_t cluster);
/*use*/static CLSTSIZE_t _FAT_tableReadSet(CLSTSIZE_t cluster, CLSTSIZE_t new_value, uint8_t task);
/*use*/static void _FAT_parse_long_names(char buff[], uint16_t sector_offset);
/*use*/static uint8_t _FAT_parse_long_names_(char buff[], uint8_t start_idx, uint16_t sector_offset, uint8_t buff_idx, uint8_t len);
...
MOUNT_RESULT FAT_mountVolume(void){
	...
	{
		sprintf(msgUart0, "%d;", temp_long.Int[0]);
		sprintf(msgUart1, "%d;", temp_long.Int[1]);
		return MR_FAT_ERR;
	} 
...
_______________________________________________________________________________________________*/
FRESULT FAT_findByIndex(DIR* dir_p, FILE_SD* finfo_p, uint16_t idx){
	FRESULT res;
...
_______________________________________________________________________________________________*/
FRESULT FAT_findNext(DIR* dir_p, FILE_SD* finfo_p){
	uint16_t idx;
...
static FRESULT _FAT_dirRegister(const char* path, uint8_t task)
{
	...
	response_code = _FAT_followPath(&dir_obj, path, task);
...
_______________________________________________________________________________________________*/
FRESULT FAT_fwriteFloat(FILE_SD* fp, float float_nr, uint8_t decimals){
...
_______________________________________________________________________________________________*/
FRESULT FAT_fwriteInt(FILE_SD* fp, INT_SIZE nr){
...
_______________________________________________________________________________________________*/
FRESULT FAT_fwriteString(FILE_SD* fp, const char* string){
	uint16_t bw;
...
_______________________________________________________________________________________________*/
FRESULT FAT_fwrite(FILE_SD* fp, const void* buff, uint16_t btw, uint16_t* bw){
	FRESULT res;
...
_______________________________________________________________________________________________*/
FRESULT FAT_fopen(DIR* dir_p, FILE_SD* file_p, char *file_name){
	FRESULT res;
...
_______________________________________________________________________________________________*/
static void _FAT_freset(FILE_SD* file_p){
	file_p->buffer_idx = 0;
	file_p->eof = false;
...
_______________________________________________________________________________________________*/
uint8_t* FAT_fread(FILE_SD* file_p){
	FRESULT res;
	uint16_t idx;
...
_______________________________________________________________________________________________*/
static uint8_t _FAT_nextFileCluster(FILE_SD* file_p){
...
_______________________________________________________________________________________________*/
static FRESULT _FAT_updateFileInfo(FILE_SD* fp, uint8_t task){
	FRESULT res;
	uint16_t idx = 0;
